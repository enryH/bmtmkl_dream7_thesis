...
names <- names(traindata[selection])
## apply to all selected real-valued datasets:
for (k in c(2,3,4,6,7,9) ) {
  i = i + 1 
  dataset <- traindata[[k]]
  total_features <- dim(dataset)[2]
  cat(total_features,"\t have been selected from dataset, " ,names(traindata)[k],"\n")
  # data_used[[i]] <- dataset[ , sort((apply(dataset, 2, myvarfct)) , decreasing=TRUE, index.return=TRUE)$ix[1:(total_features)]]
  # data_used[[i]] <- (data_used[[i]])[,1:5000]
  data_used[[i]] <- scale(dataset)
...
i= i+1
views[[i]] <- views[[1]]*views[[3]]
# Expression * Copy Number Variation (CNV)
i= i+1
views[[i]] <- views[[1]]*views[[2]]
# Copy Number Variation (CNV) * Methylation
i= i+1
views[[i]] <- views[[2]]*views[[3]]
# Expression * Copy Number Variation (CNV) * Methylation 
i= i+1
views[[i]] <- views[[1]]*views[[2]]*views[[3]] 

names(views)<- c(names(traindata[c(selection, binarydata)]), names(pathways),
			   c("Expression_Methylation", "Expression_CNV","CNV_Methylation", "Expres_CNV_Methyl")
			   )
###################################################################################################
###################################################################################################
...
####################################################################################
excluded <- -26   #only excluded drug 26
y_train <- traindata$DrugResponse[1:35,excluded] # exclude some drugs, c(-12,26,27) are not scored in wpc
...
drugResponsetest=as.matrix(read.table(filepath_testdataoutcome, header=T, row.names=1))[,excluded] 
# replaced drug 21 by drug 5...
