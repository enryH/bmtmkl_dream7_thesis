...
bmtmkl_train <- function(Km, y, parameters) {
  ...
  for (iter in 1:parameters$iteration) {
    ...
    if (parameters$progress == 1) {
      ...
      # p(a | lambda)
      for (t in 1:T) {
        lb <- lb - 0.5 * sum(diag(diag(as.vector(lambda[[t]]$shape * lambda[[t]]$scale), D[t], D[t]) %*% atimesaT.mean[[t]])) - 0.5 * (D[t] * log2pi - sum(digamma(lambda[[t]]$shape) + log(lambda[[t]]$scale)))
      }
      # p(G | a, Km, upsilon)
      for (t in 1:T) {
        lb <- lb - 0.5 * sum(diag(GtimesGT.mean[[t]])) * upsilon$shape[t] * upsilon$scale[t] + crossprod(a[[t]]$mean, KmtimesGT.mean[[t]]) * upsilon$shape[t] * upsilon$scale[t] - 0.5 * sum(diag(KmKm[[t]] %*% atimesaT.mean[[t]])) * upsilon$shape[t] * upsilon$scale[t] - 0.5 * N[t] * P * (log2pi - (digamma(upsilon$shape[t]) + log(upsilon$scale[t])))
      }
      # p(gamma)
      ...
      # p(b | gamma)
      lb <- lb - 0.5 * sum(diag(diag(as.vector(gamma$shape * gamma$scale), T, T) %*% btimesbT.mean)) - 0.5 * (T * log2pi - sum(digamma(gamma$shape) + log(gamma$scale)))
      # p(omega)
      ...
      # p(e | omega)
      lb <- lb - 0.5 * sum(diag(diag(as.vector(omega$shape * omega$scale), P, P) %*% etimeseT.mean)) - 0.5 * (P * log2pi - sum(digamma(omega$shape) + log(omega$scale)))
      # p(epsilon)
