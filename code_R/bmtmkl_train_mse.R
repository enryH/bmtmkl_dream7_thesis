...
# https://github.com/mehmetgonen/bmtmkl/blob/master/bayesian_multitask_multiple_kernel_learning_train.R

logdet <- function(Sigma) {
  2 * sum(log(diag(chol(Sigma))))
}

bmtmkl_train_mse <- function(Km, y, parameters) {
  ...
  if (parameters$progress == 1) {
    ...
    lMSE_insample_rescaled    <- matrix(0, parameters$iteration, 1)
    lMSE_insample              <- matrix(0, parameters$iteration, 1)
    lMSE_outofsample           <- matrix(0, parameters$iteration, 1)
  }
  ...
  #########################################################################################################
  if (parameters$progress == 1) {
    state <- list(lambda = lambda, upsilon = upsilon, a = a, G = G,  gamma = gamma, omega = omega, epsilon = epsilon, be = be, bounds = bounds, parameters = parameters)
  }
  else {
    state <- list(lambda = lambda, upsilon = upsilon, a = a, G = G, gamma = gamma, omega = omega, epsilon = epsilon, be = be, parameters = parameters)
  }
  ...
  lMSE_insample_rescaled[iter] <- MSE_insample_rescaled
  MSE_insample = 0
  for (t in 1:T){
    # cat(colnames(y_train)[t], "\t has ", sum(!is.na(y_train[,t])), "\t observed values. \n")
  ...
  lMSE_insample[iter] <- MSE_insample
  ...
  lMSE_outofsample[iter] <- MSE_outofsample
  #
  }
  if (parameters$progress == 1) {
    state <- list(lambda = lambda, upsilon = upsilon, a = a, G = G,  gamma = gamma, omega = omega, epsilon = epsilon, be = be, bounds = bounds, MSE_insample_rescaled = lMSE_insample_rescaled, MSE_insample = lMSE_insample, MSE_outofsample = lMSE_outofsample, parameters = parameters)
  }
  else {
    state <- list(lambda = lambda, upsilon = upsilon, a = a, G = G, gamma = gamma, omega = omega, epsilon = epsilon, be = be, parameters = parameters, MSE_insample_rescaled = MSE_insample_rescaled, MSE_insample = lMSE_insample, MSE_outofsample = lMSE_outofsample)
  }
}
