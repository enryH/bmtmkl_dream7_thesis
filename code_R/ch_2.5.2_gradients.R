...
for (o in 1:1) {
  ...
  gradient_lambda_alpha <- (gradient_lambda_alpha 
                ...
                + sum( #log p(a_t)
                        0.5 * trigamma(lambda[[o]]$shape)
                      # - 0.5 * lambda[[o]]$scale  * a_ti_sq 
                      - 0.5 * diag(atimesaT.mean[[o]] %*% diag(lambda[[o]]$scale))
                      - 0.5 * diag( 
                        diag(as.vector(lambda[[o]]$shape*lambda[[o]]$scale)) %*%
                                      (- 2 * C_at %*% a[[o]]$covariance %*% a[[o]]$covariance %*% a[[o]]$covariance 
                                                     + a[[o]]$covariance %*% a[[o]]$covariance ) %*% diag(lambda[[o]]$scale))      
                      #log q(a_t)
                      + 0.5 * diag(- a[[o]]$covariance %*% diag(lambda[[o]]$scale))
                )
                      #log p(G_t)
                 - 0.5 * upsilon$shape[o] * upsilon$scale[o] * ( 
                   - t(KmtimesGT.mean[[o]]) %*% (- a[[o]]$covariance %*% a[[o]]$covariance  %*% diag(lambda[[o]]$scale)) %*% KmtimesGT.mean[[o]] 
                   + sum(diag( KmKm[[o]] %*% (- 2 * C_at %*% a[[o]]$covariance %*% a[[o]]$covariance %*% a[[o]]$covariance 
                                                 + a[[o]]$covariance %*% a[[o]]$covariance ) %*% diag(lambda[[o]]$scale)))
                   )
...
for (o in 1:T) {
  ...
  gradient_lambda_beta <- ( gradient_lambda_beta 
                            ...
                            + sum(+ 0.5 * 1/parameters$beta_lambda^2 * lambda[[o]]$scale 
                                  - a_ti_sq * parameters$alpha_lambda * lambda[[o]]$scale^2 / parameters$beta_lambda^2
                            #log q(a)
                                  + 0.5 * diag(-a[[o]]$covariance %*% diag(as.vector(lambda[[o]]$shape*lambda[[o]]$scale^2/parameters$beta_lambda^2)))
                            + 0.5 * diag( diag(as.vector(lambda[[o]]$shape*lambda[[o]]$scale)) %*%
                              KmKm[[o]] %*% (2 * C_at %*% a[[o]]$covariance %*% a[[o]]$covariance %*% a[[o]]$covariance 
                            + a[[o]]$covariance %*% a[[o]]$covariance ) %*% diag(lambda[[o]]$scale))
                            )
  )
}
...
#
gradient_upsilon_beta  <-  (
  # log p(upsilson)  # log q(upsilon)
  GradientTheta(upsilon$shape, upsilon$scale, parameters$alpha_upsilon, parameters$beta_upsilon)
